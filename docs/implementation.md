...
- [x] Configure `tauri.conf.json` for a tray-only, borderless, transparent window `main` with dimensions 360x180.
...
- [x] Implement adaptive polling timers (500ms for live, 30s for breaks, 5m when idle).
...
- [x] Style card with premium glassmorphism dark-theme CSS in `styles.css`.
...
- [ ] Install and configure `tauri-plugin-global-shortcut` in `Cargo.toml` and `lib.rs`.
- [ ] Register `Ctrl + Alt + Space` keyboard hook at startup.
- [ ] Bind shortcut event to toggle window visibility (mirroring the tray click toggle behavior).

### Phase 5 — Auto Event Mini-Popup
- [ ] Define Event models in Rust (Wickets, Boundaries, Over complete) and parse them from Cricinfo feeds.
- [ ] Add a secondary Tauri window (`mini_popup`) configured to slide out from the screen edge.
- [ ] Write CSS and auto-dismiss timer logic (5 seconds) in the frontend for event notifications.

### Phase 6 — Multiple Match Support
- [ ] Update `parser.rs` to extract a list of all live Indian international matches.
- [ ] Extend system tray context menu in `lib.rs` to display a list of active matches.
- [ ] Store chosen match ID and adapt the background polling worker to track the chosen match.

### Phase 7 — Polish, Edge Cases & Testing
- [ ] Handle offline and connection drop states gracefully.
- [ ] Optimize memory footprint (target < 40MB active RAM).
- [ ] Perform cross-verification of cricket matches.
